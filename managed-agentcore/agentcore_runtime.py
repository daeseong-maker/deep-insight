...
def _build_graph_input(user_query: str, payload: Dict[str, Any], request_id: str = None) -> Dict[str, Any]:
    """
    ...
    - Request ID for human-in-the-loop feedback

    Args:
        user_query (str): User's query string
        payload (dict): Original request payload for additional parameters
        request_id (str): Unique request identifier for feedback tracking
    ...
    graph_input = {
        ...
        "request_id": request_id  # For human-in-the-loop feedback
    }
...
async def agentcore_streaming_execution(
    ...
    try:
        ...
        with tracer.start_as_current_span("agentcore_session") as span:
            ...
            graph = build_graph()
            graph_input = _build_graph_input(user_query, payload, request_id)

            print(f"🚀 Launching AgentCore Runtime with query: {user_query[:100]}...")

            # Step 5: Stream events from graph execution
            # Include plan_review_request and plan_review_keepalive for human-in-the-loop
            STREAM_EVENT_TYPES = {"agent_usage_stream", "agent_reasoning_stream", "agent_text_stream", "workflow_complete", "plan_review_request", "plan_review_keepalive"}
            event_count = 0
            streamed_count = 0
